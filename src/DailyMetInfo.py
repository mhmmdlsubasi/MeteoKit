...
class get():
    ...
    def _get_weather_data(self, endpoint, update_hour, update_minute, sort_order='descending'):
        now = datetime.now()
    ...
    def _control_date(self, now, update_hour, update_minute):
        ...
        if self.date is None:
            if self._is_before(now, update_hour, update_minute):
                ...
                self.date = now 
        elif self.date.date() == now.date() and self._is_before(now, update_hour, update_minute):
