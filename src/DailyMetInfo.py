...
class get():
    ...
    def get_weather_data(self, endpoint, update_hour, update_minute, sort_order='descending'):
        now = datetime.now()
        
        self.control_date(now, update_hour, update_minute)
    ...
    def control_date(self, now, update_hour, update_minute):
        if self.date:
            now = datetime.now()
            if self.date > now:
        ...
        if self.date is None:
            if self.is_before(now, update_hour, update_minute):
                self.date = (now - timedelta(days=1))
            else:
                self.date = now 
        elif self.date.date() == now.date() and self.is_before(now, update_hour, update_minute):
            raise ValueError("Belirtilen tarihe ait veri henüz yayınlanmadı.")

    def is_before(self, now, hour, minute):
        return now.hour < hour or (now.hour == hour and now.minute < minute)

    def highest_temp(self, sort_order='descending'):
        return self.get_weather_data('sondurumlar/enyuksek', 21, 37, sort_order)
    
    def lowest_temp(self, sort_order='ascending'):
        return self.get_weather_data('sondurumlar/endusuk', 9, 37, sort_order)
    
    def lowest_soil_temp(self, sort_order='ascending'):
        return self.get_weather_data('sondurumlar/toprakustu', 10, 0, sort_order)
    
    def total_precipitation(self, sort_order='descending'):
        return self.get_weather_data('sondurumlar/toplamyagis', 9, 37, sort_order)
