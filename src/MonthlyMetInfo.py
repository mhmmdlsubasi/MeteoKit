...
all_months = [datetime.strptime(str(i), "%m").strftime("%B").capitalize() for i in range(1, 13)]

class get():
    def __init__(self,province):
        province = province.strip().capitalize()    
        if province == 'Mersin': province = 'İçel' # Mersin ili, veri temin edilen adreste İçel olarak adlandırıldığı için if bloğu eklendi.
        self.province = tools.tr_to_eng(province)
    
    def general(self):
        params = {
            'k' : 'A',
            'm' : self.province.upper()
            }
    ...
    def _parse_response(self,response): 
        ...
        for row in rows:
            ...
            if keys == []:
                continue  
            key = keys[0].text.strip().replace('\n', '').replace('\r', '').replace('                 ', '') 
            if len(cells) == 1:
                df['info'] = row.text.strip()
            if len(cells) == len(columns)+1:
                del cells[-1]
                values_tuple = tuple(map(lambda x: 
                    'empty' if not x.text.strip() else # Hücrede veri olmadığı durumda 'empty' döndür.
                    float(x.text.replace(',', '.')), # Hücredeki değeri ondalık sayıya dönüştür.
                    cells))
                try:
